  ...
  while (1)
    {
      ...
      if (magic == 0x666d7420)  /* "fmt " */
      ...
      else if (magic == 0x64617461)     /* "data" */
        {
          ...
          return (BufferData*)codec (data, chunkLength, numChannels, bitsPerSample,
...
loadAUFile (InputStream *stream)
{
  ...
  switch (encoding)
    ...
    data = (char *)_alutInputStreamRead (stream, length);
  if (data == NULL)
    {
      return NULL;
    }
    return (BufferData*)codec (data, length, numChannels, bitsPerSample,
                (ALfloat) sampleFrequency);
}
...
loadRawFile (InputStream *stream)
{
  ...
  /* Guesses */
    return (BufferData*)_alutCodecLinear (data, length, 1, 8, 8000);
}
